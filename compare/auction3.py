...
class Auction3:
    ...
    def calculate_buyer_profit(self, buyer_profit):
        ...
        buyer_profit = []
        buyer_profit.append(self.rE * (self.E_loc[0] - self.E_trans[0]) + self.rC * (self.D_loc[0] - self.D_edg[0] - (self.buyer+1) * self.t) - 0.8 * self.price_paid)
        return buyer_profit
    #UAV利润
    def calculate_Ori(self, Ori):
        ...
        if len(Ori) > 0:
            return Ori
        Ori = []
        Ori.append(self.price_paid - self.rC * (self.cycles / self.Fn) - self.rE * self.k_n * self.data)
        return Ori
    ...
    def calculate_Ocoo(self, Ocoo):
        ...
        if len(Ocoo) > 0:
            return Ocoo
        Ocoo = []
        Ocoo.append(self.price_paid - self.rC * (self.cycles / self.Fn) - self.rE * (self.Pn * self.data / self.Rn[0] + self.k_n * self.data))
        return Ocoo
