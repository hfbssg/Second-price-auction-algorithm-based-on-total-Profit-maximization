...
class Auctioneer6:
    ...
    def __init__(self, average_winner_bid, average_winner_fact_bid, winner, winner_profit,
                 starting_prices=[], data=5e5, cycles=10e8, time=0.8, f_m=1e9, Pm=0.3, N_buyers=3, K_sellers=6,
    ...
    def calculate_starting_prices(self, starting_prices):
        ...
        starting_prices = []
        for seller in range(self.k_sellers):
            starting_prices.append(((self.rE * self.k_n * self.data) + (
                        (self.rC * self.cycles) / self.Fn)))  # random.random()生成0-1之间的随机浮点数
    ...
    def choose_winner(self, bids):  # 保证总利润最高
        """
        ...
        price_to_pay = valid_bids[0]  # 支付第二出价 price_to_pay = valid_bids[0] #支付第一出价
    ...
    def start_auction(self, starting_prices):
        ...
        for seller in range(self.k_sellers):  # 10个卖家
            ...
            seller_bid = auction2.seller_profit[0]  # 实际卖家利润
            buyer_bid = auction2.buyer_profit[0]  # 实际赢家利润
            total[seller] = seller_bid + buyer_bid
            ...
            if seller == 0:
                ...
                self.cumulative_total_profits[seller] = total[seller] + total[seller - 1] + total[seller - 2] + total[
                    seller - 3] + total[seller - 4]

            elif seller == 5:
                self.cumulative_total_profits[seller] = total[seller] + total[seller - 1] + total[seller - 2] + total[
                    seller - 3] + total[seller - 4] + total[seller - 5]
                ...
                self.cumulative_total_profits[seller] = total[seller] + total[seller - 1] + total[seller - 2] + total[
                    seller - 3] + total[seller - 4] + total[seller - 5] + total[seller - 6] + total[seller - 7] + total[
                                                            seller - 8]

            elif seller == 9:
                self.cumulative_total_profits[seller] = total[seller] + total[seller - 1] + total[seller - 2] + total[
                    seller - 3] + total[seller - 4] + total[seller - 5] + total[seller - 6] + total[seller - 7] + total[
                                                            seller - 8] + total[seller - 9]

            else:
                self.cumulative_total_profits[seller] = total[seller] + total[seller - 1] + total[seller - 2] + total[
                    seller - 3] + total[seller - 4] + total[seller - 5] + total[seller - 6] + total[seller - 7] + total[
                                                            seller - 8] + total[seller - 9] + total[seller - 10]

    def store_auction_history(self, starting_price, buyer, price_paid, bid_history, auction_round, data, cycles, f_m,
                              Pm):
        """
...
\       """
        ...
        plt.figure()
        for seller in range(self.k_sellers):
            plt.semilogy(self.cumulative_sellers_profits[seller],
                         label="Seller " + str(seller))  # matplotlib.pyplot.semilogy()函数用于绘制在y轴上具有对数刻度的图。
...
if __name__ == '__main__':
    ...
    # strategy[0] = 4
    auctioneer = Auctioneer6(average_winner_bid=[], average_winner_fact_bid=[], winner=[], winner_profit=[],
                             starting_prices=[], data=5e5, cycles=10e8, time=0.8, f_m=1e9,
                             Pm=0.3, N_buyers=3, K_sellers=10,
                             R_rounds=20, rE=0.1, rC=60, k_buyers=10e-27, debug=True, w=10e6, Gm=300, n0=10, delay=0.02,
                             Fn=5e9, t=0.01,
