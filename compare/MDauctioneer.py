...
class Auctioneer3:
    ...
    def __init__(self, average_winner_bid, average_winner_fact_bid, winner, winner_profit, cumulative_total_profits, starting_prices=[], data=5e5, cycles=10e8, time=0.8, f_m=1e9, Pm=0.3, N_buyers=10, K_sellers=3,
        ...
        self.cumulative_total_profits = cumulative_total_profits
    ...
    def choose_winner(self, buyer, bids, total_profit): #保证总利润最高
        """
        ...
        winner_id = [key1 for key1 in range(buyer+1) if total_profit[key1] == total[0]][0] #总利润最高为赢家

        if valid_bids[winner_id] == fact_bids[len(fact_bids)-1]:
            price_to_pay = valid_bids[winner_id]  # 赢家第二支付规则
        else:
            id = [key2 for key2 in range(buyer+1) if fact_bids[key2] == valid_bids[winner_id]][0]
...
#         print("------------------------------------------------------")
    ...
    def update_profits(self, auction_round): #实际利润
        ...
        """
        seller = 0
        self.sellers_total_profits[auction_round] = 0
        self.buyers_total_profits[auction_round] = 0
        for auction in self.auctions_history[auction_round]:
            self.buyers_profits[auction_round, self.winner[seller]] += self.winner_profit[seller]
    ...
    def start_auction(self, starting_prices):
        ...
        for buyer in range(self.n_buyers):  # 10个买家
            if buyer == 0:
                ...
                auction1 = Auction2(starting_price=starting_price, buyer=buyer, price_paid=buyers_bid[buyer],
                ...
                total_profit[0] = total_bid_profit
                self.cumulative_total_profits.append(total_bid_profit)  # 根据投标计算每个MD的总利润
            else:
                bid = self.calculate_bid(buyer, starting_price)
                buyers_bid[buyer] = bid
                auction3 = Auction2(starting_price=starting_price, buyer=buyer, price_paid=buyers_bid[buyer],
                                   bid_history=buyers_bid, data=self.data,
                                   cycles=self.cycles, f_m=self.f_m, Pm=self.Pm)
                ...
                winner, price_to_pay = self.choose_winner(buyer, buyers_bid, total_profit)


                auction4 = Auction2(starting_price=starting_price, buyer=winner, price_paid=price_to_pay,
                                   bid_history=buyers_bid, data=self.data,
                                   cycles=self.cycles, f_m=self.f_m, Pm=self.Pm)

                seller_bid = auction4.seller_profit[0]  # 实际卖家利润
                buyer_bid = auction4.buyer_profit[0]  # 实际赢家利润

                total_bid_profit = seller_bid + buyer_bid
                cumulative_total_bid_profit = total_bid_profit + self.cumulative_total_profits[buyer-1]
                self.cumulative_total_profits.append(cumulative_total_bid_profit)  # 根据投标计算整个拍卖的实际总利润

        # for buyer in range(self.n_buyers):
        #     self.cumulative_buyers_profits[buyer] = self.cumulative_buyers_profits[
        #                                                                buyer, auction_round - 1] + self.buyers_profits[
        #                                                                auction_round, buyer]
        #     self.buyers_total_profits[buyer] += self.cumulative_buyers_profits[buyer]
        #
    ...
    def store_auction_history(self, starting_price, buyer, price_paid, bid_history, auction_round, data, cycles, f_m, Pm):
        ...
        """
        auction = Auction2(starting_price, buyer, price_paid, bid_history, auction_round, data, cycles, f_m, Pm)
...
if __name__ == '__main__':
    ...
    # strategy[0] = 4
    auctioneer = Auctioneer3(average_winner_bid=[], average_winner_fact_bid=[], winner=[], winner_profit=[], starting_prices=[], cumulative_total_profits=[], data=5e5, cycles=10e8, time=0.8, f_m=1e9, Pm=0.3, N_buyers=10, K_sellers=3,
