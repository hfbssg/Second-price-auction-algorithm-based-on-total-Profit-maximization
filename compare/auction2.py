...
class Auction2:
    ...
    def __init__(self, starting_price, buyer, price_paid, bid_history, data, cycles, f_m, Pm, N_buyers=3, K_sellers=3, seller_profit=[],
                 rE=0.1, rC=60, Pn=3, Fn=5e9, w=10e6, Gm=300, n0=10, k_buyers=10e-27, delay=0.02, t=0.1, D_loc=[], E_loc=[], D_ori=[],
                 D_coo=[], D_edg=[], E_trans=[], Ori=[], Ocoo=[], Rn=[], k_sellers=10e-6, i=[], buyer_profit=[]):
        """
        ...
        self.buyer_profit = self.calculate_buyer_profit(buyer_profit)
    ...
    def calculate_E_trans(self, E_trans):
        ...
        return E_trans
    #MD赢家利润
    def calculate_buyer_profit(self, buyer_profit):
        """
         Calculate winner_profit //计算赢家实际利润
        """
        if len(buyer_profit) > 0:
            return buyer_profit
        # winner_profit = np.zeros((auction_round, 1))
        # for auction in range(0, self.auction_round):
        buyer_profit = []
        buyer_profit.append(self.rE * (self.E_loc[0] - self.E_trans[0]) + self.rC * (self.D_loc[0]- self.D_edg[0]) - 0.8*self.price_paid)
        return buyer_profit
    #UAV利润
    def calculate_Ori(self, Ori):
        ...
        if len(Ori) > 0:
            return Ori
        Ori = []
        Ori.append(self.price_paid - self.rC * ((self.cycles / self.Fn) + (self.buyer+1) * self.t) - self.rE * self.k_n * self.data)
        return Ori
    ...
    def calculate_Ocoo(self, Ocoo):
        ...
        if len(Ocoo) > 0:
            return Ocoo
        Ocoo = []
        Ocoo.append(self.price_paid - self.rC * ((self.cycles / self.Fn) + (self.buyer+1) * self.t) - self.rE * (self.Pn * self.data / self.Rn[0] + self.k_n * self.data))
        return Ocoo
