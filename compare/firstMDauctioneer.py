...
class Auctioneer2:
    ...
    def __init__(self, winner, average_winner_bid, average_winner_fact_bid, cumulative_total_profits=[], starting_prices=[], data=5e5, cycles=10e8, time=0.8, f_m=1e9, Pm=0.3, N_buyers=10, K_sellers=3,
...
#         print("------------------------------------------------------")
    ...
    def start_auction(self, starting_prices):
        ...
        for buyer in range(self.n_buyers):  # 10个买家
            if buyer == 0:
                ...
                winner = buyer
                price_to_pay = buyers_bid[buyer]
                auction1 = Auction2(starting_price=starting_price, buyer=winner, price_paid=price_to_pay,
                                    bid_history=buyers_bid, data=self.data,
                                    cycles=self.cycles, f_m=self.f_m, Pm=self.Pm)

                seller_bid = auction1.seller_profit[0]  # 实际卖家利润
                buyer_bid = auction1.buyer_profit[0]  # 实际赢家利润

                total_bid_profit = seller_bid + buyer_bid
                cumulative_total_bid_profit = cumulative_total_bid_profit + total_bid_profit
                self.cumulative_total_profits.append(cumulative_total_bid_profit)
            else:
                bid = self.calculate_bid(buyer, starting_price)
                buyers_bid[buyer] = bid
    ...
    def store_auction_history(self, starting_price, winner, price_paid, bid_history, auction_round, data, cycles, f_m, Pm):
        ...
        """
        auction2 = Auction2(starting_price, winner, price_paid, bid_history, auction_round, data, cycles, f_m, Pm)
        self.auctions_history[auction_round].append(auction2)
        return auction2
...
if __name__ == '__main__':
    ...
    # strategy[0] = 4
    auctioneer = Auctioneer2(winner=[], average_winner_bid=[], average_winner_fact_bid=[], cumulative_total_profits=[], starting_prices=[], data=5e5, cycles=10e8, time=0.8, f_m=1e9, Pm=0.3, N_buyers=10, K_sellers=3,
