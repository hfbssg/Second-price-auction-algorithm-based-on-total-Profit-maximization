...
class Auctioneer3:
    ...
    def __init__(self, average_winner_bid, average_winner_fact_bid, winner, starting_prices=[], data=5e5, cycles=10e8, time=0.8, f_m=1e9, Pm=0.3, N_buyers=3, K_sellers=3,
        ...
        self.cumulative_buyers_profits2 = np.zeros((self.n_buyers, 20))
...
#         print("------------------------------------------------------")
    ...
    def update_profits(self, auction_round): #实际利润
        ...
        seller = 0
        for auction in self.auctions_history[auction_round]:
            self.buyers_profits[auction_round, self.winner[seller+self.k_sellers*auction_round]] += auction.winner_profit[0]
            self.sellers_profits[auction_round, seller] += auction.seller_profit[0]
            seller += 1
    ...
    def start_auction(self, starting_prices):
        ...
        buyers_bid = {}
        for auction_round in range(self.r_rounds): #20轮
            winner_total = 0
            winner_fact_total = 0
            ...
            for seller in range(self.k_sellers): #3个卖家
                ...
                for buyer in range(self.n_buyers): #3个买家
                    if auction_round != 0 and buyer != self.winner[seller + self.k_sellers * (auction_round - 1)]: #确保是同一个卖家的赢家
                        ...
                        buyers_bid[winner] = winner_bid #赢家投标保持不变
                    else:
                        bid = self.calculate_bid(seller * self.n_buyers + buyer, starting_price)
                        buyers_bid[buyer] = bid

                    auction1 = Auction1(starting_price=starting_price, buyer=buyer, winner=buyer, price_paid=buyers_bid[buyer],
                                      bid_history=buyers_bid, auction_round=auction_round, data=self.data,
                                      cycles=self.cycles, f_m=self.f_m, Pm=self.Pm)

                    seller_bid = auction1.seller_profit[0] #根据投标计算卖家利润 不是实际利润
                    seller_profit[buyer] = seller_bid

                    buyer_bid = auction1.buyer_profit[0] #根据投标计算买家利润
                    buyer_profit[buyer] = buyer_bid
                ...
                winner, price_to_pay = self.choose_winner(buyers_bid, total_profit) #根据总利润最高确定赢家，其的支付为实际支付
    ...
    def store_auction_history(self, starting_price, winner, buyer, price_paid, bid_history, auction_round, data, cycles, f_m, Pm):
        """
        ...
        """
        auction = Auction1(starting_price, winner, buyer, price_paid, bid_history, auction_round, data, cycles, f_m, Pm)
...
\       """
        ...
        # for seller in range(self.k_sellers):
        #     plt.semilogy(x, self.cumulative_sellers_profits[seller], label="Seller " + str(seller)) #matplotlib.pyplot.semilogy()函数用于绘制在y轴上具有对数刻度的图。
        ...
        for seller in range(self.k_sellers):
            y_1 = self.cumulative_sellers_profits[seller]
            plt.plot(x, y_1, linewidth=1, linestyle='-', label="卖家 " + str(seller), marker="o", markersize=4)
        ...
        plt.figure(figsize=(8, 6))
        # for buyer in range(self.n_buyers):
        #     self.cumulative_buyers_profits2[buyer] = self.cumulative_buyers_profits[buyer][:7]
        #     plt.semilogy(x1, self.cumulative_buyers_profits2[buyer], label="Buyer " + str(buyer), linewidth=2, linestyle='-', marker ="o", markersize=5)
        ...
        plt.grid(True, linestyle='--', alpha=0.5)
        for buyer in range(self.n_buyers):
            self.cumulative_buyers_profits2[buyer] = self.cumulative_buyers_profits[buyer][:20]
            y_2 = self.cumulative_buyers_profits2[buyer]
            plt.plot(x1, y_2, linewidth=1, linestyle='-', label="买家" + str(buyer), marker ="o", markersize=4)
        ...
        #     plt.semilogy(x, self.cumulative_total_profits, color='green',linewidth=1, linestyle='-', marker = "o", markersize=4)
        ...
        plt.grid(True, linestyle='--', alpha=0.5)
        plt.plot(x, y_3, color='green', linewidth=1, linestyle='-',  label='积累总利润', marker = "o", markersize=4)
        plt.legend()  # 显示图例
        plt.ylabel('积累总利润')
        plt.xlabel('轮数')
        plt.legend()
...
if __name__ == '__main__':
    ...
    # strategy[0] = 4
    auctioneer = Auctioneer3(average_winner_bid=[], average_winner_fact_bid=[], winner=[], starting_prices=[], data=5e5, cycles=10e8, time=0.8, f_m=1e9, Pm=0.3, N_buyers=3, K_sellers=3,
                            R_rounds=20, rE=0.1, rC=60, k_buyers=10e-27, debug=True, w=10e6, Gm=300, n0=10, delay=0.02,Fn=5e9, t=0.001,
