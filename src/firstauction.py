            ...
            for seller in range(self.k_sellers): #3个卖家
                ...
                for buyer in range(self.n_buyers): #3个买家
                    if auction_round != 0 and buyer != self.winner[seller + self.k_sellers * (auction_round - 1)]: #确保是同一个卖家的赢家
                        ...
                        buyers_bid[winner] = winner_bid * np.random.uniform(0.7, 0.9)#赢家降低投标，不低于卖家的初始要价
                    else:
...
if __name__ == '__main__':
    ...
    # strategy[0] = 4
    auctioneer = Auctioneer2(winner=[], average_winner_bid=[], average_winner_fact_bid=[], starting_prices=[], data=5e5, cycles=10e8, time=0.8, f_m=1e9, Pm=0.3, N_buyers=3, K_sellers=3,
