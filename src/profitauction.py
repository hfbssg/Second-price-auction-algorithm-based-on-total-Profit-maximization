...
class Auction1:
    ...
    def __init__(self, starting_price, buyer, winner, price_paid, bid_history, auction_round, data, cycles, f_m, Pm, N_buyers=3, K_sellers=3, winner_profit=[], seller_profit=[],
    ...
    def calculate_buyer_profit(self, buyer_profit):
        """
         Calculate winner_profit //计算买家实际利润
        """
        ...
        buyer_profit = []
        buyer_profit.append(self.rE * (self.E_loc[0] - self.E_trans[0]) + self.rC * (self.D_loc[0] - self.D_edg[0] - (self.auction_round+1) * self.t) - 0.8*self.price_paid)
        return buyer_profit
